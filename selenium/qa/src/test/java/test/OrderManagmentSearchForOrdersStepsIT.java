...
public class OrderManagmentSearchForOrdersStepsIT extends BaseSteps {
	...
	@Test(dataProvider = "guest_page_search_data", priority = 13, dependsOnMethods = {
	...
	@Test(dataProvider = "order_manage_page_orders_navigation", priority = 14, retryAnalyzer = utils.RetryAnalizer.class)
	public void b_manageOrderNavigateToOrders(User orgAdmin) throws Exception {
		...
		loginStepsFacade.givenAdminUserIsLogedIn(orgAdmin);
		organizationFacade.givenOrganizationExist(purchase.getEvent().getOrganization());
		adminEventFacade.givenUserIsOnAdminEventsPage();
		AdminEventComponent eventComponent = adminEventFacade.findEventWithName(purchase.getEvent());

		Assert.assertNotNull(eventComponent, "No Event with name: " + purchase.getEvent().getEventName() + " found");
		eventComponent.clickOnDashboardOption();
		dashboardFacade.thenUserIsOnEventDashboardPage();
		dashboardFacade.whenUserSelectsManageOrdersFromOrdersDropDown();

		Integer numberOrOrders = dashboardFacade.thenNumberOfOrdersIs();
		Assert.assertEquals(numberOrOrders, NUMBER_OF_ORDERS);
		loginStepsFacade.logOut();
	}

	@DataProvider(name = "order_manage_page_orders_navigation")
	public static Object[][] pageNavigationManageOrdersData() {
		User orgAdminUser = User.generateUserFromJson(DataConstants.ORGANIZATION_ADMIN_USER_KEY);
		return new Object[][] { { orgAdminUser } };
	}
	...
	public void c_manageOrdersPageSearchTest(User orgAdmin, User customer, User customerTwo, Event event)
			throws Exception {
	...
	@Test(dataProvider = "manage_orders_page_search_data", priority = 14, retryAnalyzer = utils.RetryAnalizer.class)
	public void c_manageOrdersSearchAndRefundTickets(User orgAdmin, User customer, User customerTwo, Event event)
			throws Exception {
		LoginStepsFacade loginFacade = new LoginStepsFacade(driver);
		AdminEventStepsFacade adminEventFacade = new AdminEventStepsFacade(driver);
		OrganizationStepsFacade organizationFacade = new OrganizationStepsFacade(driver);
