...
public class CreateDraftEvent extends BaseSteps {
	
	
	@Test(dataProvider = "event_save_as_draft", priority = 11)
	public void createDraftEvent(User superuser, Event event) throws Exception {
		...
		Assert.assertTrue(isEventSavedToDraft, "1.Event: " + event.getEventName() + " is not saved to draft");
		adminEventFacade.givenUserIsOnAdminEventsPage();
