...
public class AdminEventsPage extends BasePage {
	...
	private List<WebElement> findWebElementsEventByName(String name) {
		...
		List<WebElement> list = explicitWait(15, ExpectedConditions.visibilityOfAllElementsLocatedBy(findByNameXpath(name)));
		return list;
	}
	
	private WebElement findWebElementEventByName(String name) {
		explicitWaitForVisiblity(eventsDashboardContainer);
		WebElement event = explicitWait(15, ExpectedConditions.visibilityOfElementLocated(findByNameXpath(name)));
		return event;
	}
